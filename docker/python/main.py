...
conn = psycopg2.connect(host="localhost", user="admin", password="admin", dbname="dataproj", port="321")
curs = conn.cursor()
...
with open('../elect/trump.csv', 'r') as csvfile:
    ...
    for row in reader:
            try:
                ...
                continue
with open('/home/artem/docker/hdfs/datanode/ods/trump.csv', 'w', newline='') as f:
        w = csv.writer(f)
...
with open('../elect/biden.csv', 'r') as csvfile:
    reader = csv.reader(csvfile)
    data = []
    for row in reader:
            try:
                ...
                continue
with open('/home/artem/docker/hdfs/datanode/ods/biden.csv', 'w', newline='') as f:
        w = csv.writer(f)
        w.writerow(('tweet_id', 'user_id', 'state_code'))
        w.writerows(data)
f.close()
...
with open('../elect/states.csv', 'r') as csvfile:
    reader = csv.reader(csvfile)
    data = []
    fl = True
    for row in reader:
...
with open('/home/artem/docker/hdfs/datanode/ods/states.csv', 'w', newline='') as f:
        w = csv.writer(f)
...
with open('../elect/votes.csv', 'r') as csvfile:
...
with open('/home/artem/docker/hdfs/datanode/ods/votes.csv', 'w', newline='') as f:
        w = csv.writer(f)
